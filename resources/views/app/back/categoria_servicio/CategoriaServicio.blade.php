    ...
    <div class="container-fluid">
        ...
        <div class="card border-0 shadow-sm p-4 bg-white">
            <div class="d-flex justify-content-between align-items-center mb-4">
                <h4 class="fw-bold text-secondary m-0">Cancelacion de servicios </h4>
                <a href="{{ route('categoriaServicio.create') }}" class="btn btn-info text-white shadow-sm fw-bold">
                    <i class="bi bi-person-plus-fill me-2"></i> NUEVO CATEGORIA DE SERVICIO
                </a>
            </div>
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td class="text-center">
                                <span class="badge bg-light text-dark border">{{ $categoria_servicio->id }}</span>
                            </td>
                            <td class="fw-bold">{{ $categoria_servicio ->categoria }}</td>
                            <td>{{ $categoria_servicio->descripcion }}</td>
                            
                            <td>
                                <div class="d-flex gap-2 justify-content-center">
                                    <a href="{{ route('categoriaServicio.edit', $categoria_servicio) }}" class="text-warning">
                                        <i class="bi bi-pencil-square fs-5"></i>
                                    </a>
                                    <form method="POST" action="{{ route('categoriaServicio.destroy', $categoria_servicio) }}" onsubmit="return confirm('¿Desea eliminar esta categoría de servicio?')">
                                        @csrf 
                                        @method('DELETE')
